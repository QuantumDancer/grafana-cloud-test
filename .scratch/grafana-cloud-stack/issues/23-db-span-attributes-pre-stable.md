...
2026-08-07: Resolved (commit `fd30bc8`, branch `worktree-issues-20-23`) exactly as
specified — the two env vars, chart-driven, opt-in mirrored as a Dockerfile default (the
peer mapping deliberately not: it references in-cluster DNS). Deployed as helm revision 9
at 13:53:48Z UTC via `helm get values` + `helm upgrade -f` from the worktree
(deploy-shop.sh needs tofu state the worktree doesn't have; `--reuse-values` avoided
since it would skip the new chart defaults).

Verified live by a fresh-context verifier, 13:56–14:00Z, all three symptoms:
- JDBC spans post-13:54Z carry `peer.service=shop-db`, and Mimir already shows
  `traces_service_graph_request_total{client="spyglass-backend", server="shop-db",
  connection_type="database"}` — the hosted metrics-generator does follow the OSS
  resolution order, and `/dup` preserved the database classification as intended (the
  old `shop-db-rw.shop` node is aging out of the window).
- Dup mode confirmed on a real span: legacy (`db.system`, `db.name`, `db.statement`,
  `db.sql.table`) and stable (`db.system.name`, `db.namespace`, `db.query.text`,
  `db.query.summary`) attribute generations present simultaneously.
- The N+1 waterfall (`GET /api/customers/{id}/orders`, 47 DB spans) now names children
  per table: 34× `select products`, 10× `select order_items` — the uniform `SELECT shop`
  is gone. Note the names are lowercase (`select products`, not `SELECT products`):
  `db.query.summary` mirrors the agent's lowercase SQL; the done-criterion's substance
  (per-table naming) is met. One cosmetic leftover per request: the empty-statement JDBC
  connection span is now named `shop|shop` (namer falls back to `db.namespace`, which in
  dup mode is the catalog|schema pair) — previously the equally meaningless `shop`.

Symptom 3's trace→query linking remains unverifiable from public docs (as anticipated
above); the spans now satisfy a current-semconv implementation should one appear.
